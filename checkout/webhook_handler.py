from django.http import HttpResponse
from django.core.mail import send_mail
from django.template.loader import render_to_string
from django.conf import settings
from .models import Order, OrderLineItem
from user_profile.models import Address
from supplies.models import Supply
from user_profile.models import UserProfile
import json
import time


class StripeWH_Handler:
    """Handle Stripe webhooks"""

    def __init__(self, request):
        self.request = request

    def _send_confirmation_email(self, order):
        """Send the user a confirmation email"""
        cust_email = order.email
        subject = render_to_string(
            'checkout/confirmation_emails/confirmation_email_subject.txt',
            {'order': order})
        body = render_to_string(
            'checkout/confirmation_emails/confirmation_email_body.txt',
            {'order': order, 'contact_email': settings.DEFAULT_FROM_EMAIL})

        send_mail(
            subject,
            body,
            settings.DEFAULT_FROM_EMAIL,
            [cust_email]
        )

    def handle_event(self, event):
        """
        Handle a generic/unknown/unexpected webhook event
        """
        return HttpResponse(
            content=f'Unhandled webhook received: {event["type"]}',
            status=200)

    def handle_payment_intent_succeeded(self, event):
        """
        Handle the payment_intent.succeeded webhook from Stripe
        """
        intent = event.data.object
        pid = intent.id
        bag = intent.metadata.bag
        save_info = intent.metadata.save_info

        billing_details = intent.billing_details
        shipping_details = intent.shipping
        grand_total = round(intent.amount / 100, 2)

        # Clean data in the shipping details
        for field, value in shipping_details.address.items():
            if value == "":
                shipping_details.address[field] = None

        # Update profile information if save_info was checked
        profile = None
        delivery_address = None
        username = intent.metadata.username
        is_default_address = save_info.lower() in ['true', '1', 'on']
        if username != 'AnonymousUser':
            profile = UserProfile.objects.get(user__username=username)
            if is_default_address:
                profile.phone_number = shipping_details.phone
                profile.save()

                delivery_address = Address.objects.create(
                    country=shipping_details.address.country,
                    postcode=shipping_details.address.postal_code,
                    town_or_city=shipping_details.address.city,
                    street_address1=shipping_details.address.line1,
                    street_address2=shipping_details.address.line2,
                    county=shipping_details.address.state,
                    is_default=is_default_address,
                    user=profile.user,
                    type=1)

        order_exists = False
        attempt = 1
        while attempt <= 5:
            try:
                order = Order.objects.get(
                    full_name__iexact=shipping_details.name,
                    email__iexact=billing_details.email,
                    phone_number__iexact=shipping_details.phone,
                    delivery_address__country__iexact=shipping_details.address.country,
                    delivery_address__postcode__iexact=shipping_details.address.postal_code,
                    delivery_address__town_or_city__iexact=shipping_details.address.city,
                    delivery_address__street_address1__iexact=shipping_details.address.line1,
                    delivery_address__street_address2__iexact=shipping_details.address.line2,
                    delivery_address__county__iexact=shipping_details.address.state,
                    grand_total=grand_total,
                    original_bag=bag,
                )
                order_exists = True
                break
            except Order.DoesNotExist:
                attempt += 1
                time.sleep(2)
        if order_exists:
            self._send_confirmation_email(order)
            return HttpResponse(
                content=f'Webhook received: {event["type"]} | SUCCESS: Verified order already in database',
                status=200)
        else:
            order = None
            try:
                billing_address = Address.objects.create(
                    country=billing_details.address.country,
                    postcode=billing_details.address.postal_code,
                    town_or_city=billing_details.address.city,
                    street_address1=billing_details.address.line1,
                    street_address2=billing_details.address.line2,
                    county=billing_details.address.state,
                    type=0,
                    user=profile.user if profile and profile.user else None,
                    is_default=is_default_address
                    )

                if not delivery_address:
                    delivery_address = Address.objects.create(
                        country=shipping_details.address.country,
                        postcode=shipping_details.address.postal_code,
                        town_or_city=shipping_details.address.city,
                        street_address1=shipping_details.address.line1,
                        street_address2=shipping_details.address.line2,
                        county=shipping_details.address.state,
                        type=1,
                        user=profile.user if profile and profile.user else None,
                        is_default=is_default_address
                        )

                order = Order.objects.create(
                    full_name=shipping_details.name,
                    user_profile=profile,
                    email=billing_details.email,
                    phone_number=shipping_details.phone,
                    billing_address=billing_address,
                    delivery_address=delivery_address,
                    grand_total=grand_total,
                    original_bag=bag,
                    stripe_pid=pid,
                )
                for item_id, item_data in json.loads(bag).items():
                    supply = Supply.objects.get(id=item_id)
                    for renting_date, date_item in item_data.items():
                        for renting_days, days_item in date_item.items():
                            order_line_item = OrderLineItem(
                                order=order,
                                supply=supply,
                                price_per_day=supply.price_per_day,
                                quantity=int(days_item['quantity']),
                                renting_days=int(renting_days),
                                start_renting_date=renting_date,
                            )
                            order_line_item.save()
            except Exception as e:
                if order:
                    order.delete()
                return HttpResponse(
                    content=f'Webhook received: {event["type"]} | ERROR: {e}',
                    status=500)
        self._send_confirmation_email(order)
        return HttpResponse(
            content=f'Webhook received: {event["type"]}',
            status=200)

    def handle_payment_intent_payment_failed(self, event):
        """
        Handle the payment_intent.payment_failed webhook from Stripe
        """

        return HttpResponse(
            content=f'Webhook received: {event["type"]}',
            status=200)
